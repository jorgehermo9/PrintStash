...
from app.modules.work.catalog import INGEST, RECONCILE_DEFINITION
...
class TestOverview:
    def test_lists_every_lane_with_its_depth(
        ...
        assert (lanes[INGEST]["queued"], lanes[INGEST]["running"]) == (1, 0)
        assert lanes[INGEST]["overridden"] is False
    ...
    def test_counts_failed_derivatives(
        ...
        make_derivative(artifact, THUMBNAIL, state=DerivativeState.FAILED)
...
class TestUpdateLane:
    def test_changes_a_lanes_concurrency_at_runtime(
        ...
        response = client.put(
            f"/api/v1/admin/work/lanes/{INGEST}",
            headers=_headers(admin),
            json={"concurrency": 5},
        )

        assert response.status_code == 200, response.text
        lane = next(lane for lane in response.json()["lanes"] if lane["name"] == INGEST)
        assert (lane["concurrency"], lane["overridden"]) == (5, True)
        assert work_engine.concurrency[INGEST] == 5
        # Persisted where the engine's disposable state cannot lose it.
        row = db_session.get(WorkLaneOverride, INGEST)
    ...
    def test_null_restores_the_configured_concurrency(
        ...
        assert db_session.get(WorkLaneOverride, INGEST) is None

    def test_an_unknown_lane_is_not_found(
        self, client: TestClient, admin: User
    ) -> None:
        response = client.put(
        ...
        assert response.json()["detail"] == "lane_not_found"
...
class TestCancelQueued:
    ...
    def test_an_unknown_definition_is_not_found(
        self, client: TestClient, admin: User
    ) -> None:
        response = client.post(
            "/api/v1/admin/work/cancel-queued",
...
class TestRegenerateDerivatives:
    def test_missing_only_nudges_the_producers(
        ...
        response = client.post(
            f"/api/v1/admin/work/derivatives/{THUMBNAIL}/regenerate",
            headers=_headers(admin),
            json={"mode": "missing"},
        )
    ...
    def test_all_marks_every_output_of_the_kind_stale(
        ...
        assert response.json() == {"kind": THUMBNAIL, "mode": "all"}
        row = db_session.get(DerivativeRegeneration, THUMBNAIL)
        assert row is not None
