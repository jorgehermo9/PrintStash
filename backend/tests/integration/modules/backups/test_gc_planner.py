...
from app.modules.storage.storage_identity import independent_evidence, s3_target
...
class TestApproveGcPlan:
    ...
    def test_unknown_target_identity_cannot_authorize_gc(
        ...
        monkeypatch.setattr(
            backup_verification,
            "verify_backup",
            lambda *_args, **_kwargs: backup_contracts.BackupVerification(
    ...
    def test_backup_witness_requires_recent_verified_independent_s3_source(
        ...
        monkeypatch.setattr(backup_catalogue, "list_backup_sources", lambda: candidates)
        monkeypatch.setattr(
            backup_targets,
        ...
        monkeypatch.setattr(
            backup_verification,
            "verify_backup",
            lambda backup_id, *, source_ref: backup_contracts.BackupVerification(
                backup_id=backup_id,
                valid=True,
                app_compatible=True,
                manifest_version="3",
...
class TestFinalizeGcPlan:
    ...
    def test_unexpected_finalization_failure_blocks_with_evidence(
        ...
        monkeypatch.setattr("app.modules.library.trash.hard_delete_model", crash)
...
class TestBackupReverification:
    ...
    def test_missing_or_invalid_backup_source_fails_closed(
        ...
        run = self._run()
        monkeypatch.setattr(gc_planner, "_active_provider_ref", lambda: "1" * 64)
        monkeypatch.setattr(backup_catalogue, "list_backup_sources", lambda: [])
...
class TestScheduledGc:
    def test_scheduler_pauses_during_restore_or_disabled_retention(
        ...
        monkeypatch.setattr(backup_maintenance, "restore_in_progress", lambda: True)
        assert gc_planner.run_scheduled_gc(retention_days=30)["gc_candidates"] == 0

        monkeypatch.setattr(backup_maintenance, "restore_in_progress", lambda: False)
