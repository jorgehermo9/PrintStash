...
class TestRunDispatcherLoop:
    ...
    @pytest.mark.asyncio
    async def test_run_dispatcher_loop_survives_tick_error(self, db_session):
    ...
    @pytest.mark.asyncio
    async def test_run_dispatcher_loop_reraises_cancelled_error_from_tick(self):
        with (
            patch.object(notifications, "_POLL_INTERVAL_S", 0.01),
            patch.object(
                notifications, "dispatch_due", side_effect=asyncio.CancelledError
            ),
        ):
            task = asyncio.create_task(notifications.run_dispatcher_loop())
            with pytest.raises(asyncio.CancelledError):
