...
class TestGetBackend:
    def test_get_backend_requires_explicit_binding(
        ...
        with pytest.raises(RuntimeError, match="storage_backend_not_bound"):
            storage_runtime.get_backend()

        bound = LocalStorageBackend()
        assert storage_runtime.bind_backend(bound) is bound
        assert storage_runtime.get_backend() is bound
...
class TestCreateBackend:
    def test_create_backend_selects_adapter_without_binding(
        ...
        monkeypatch.setattr(storage_runtime, "LocalStorageBackend", _Local)
        monkeypatch.setattr(storage_runtime, "S3StorageBackend", _S3)

        assert isinstance(storage_runtime.create_backend("local"), _Local)
        assert isinstance(storage_runtime.create_backend("s3"), _S3)
        assert isinstance(storage_runtime.create_backend("unexpected"), _Local)
...
class TestInitBackend:
    def test_init_backend_validates_before_binding(
        ...
        backend = _Backend()
        monkeypatch.setattr(storage_runtime, "_backend", None)
        monkeypatch.setattr(storage_runtime, "create_backend", lambda _name: backend)

        assert storage_runtime.init_backend() is backend
