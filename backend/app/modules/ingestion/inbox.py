...
from app.core.provider_redaction import redact_url
...
def validate_import_selection(row: InboxItem, requested: list[str]) -> list[str]:
    ...
    if (
        ...
        raise OperationError("file_selection_invalid", kind=ErrorKind.UNPROCESSABLE)
    return selected
...
def require_visible(session: Session, user: User, item_id: int) -> InboxItem:
    ...
    if row is None or (not user.is_superuser and row.owner_user_id != user.id):
        raise OperationError("pending_import_not_found", kind=ErrorKind.NOT_FOUND)
    return row
...
def prune_expired_browser_leases() -> int:
    ...
    with get_session_factory().scoped_session() as session:
        ...
        for inbox_item_id, job_owned in expired_slot_item_ids.items():
            ...
            session.add(row)
            expired_items += 1
        staging_cleanup.prune_expired(session)
...
def require_capture_slot(
    ...
    slot = session.get(CaptureUploadSlot, slot_id)
    if slot is None:
        raise OperationError("capture_upload_slot_not_found", kind=ErrorKind.NOT_FOUND)
...
def finalize_capture_upload(session: Session, user: User, item_id: int) -> InboxItem:
    ...
    if (
        ...
        raise OperationError("capture_upload_not_finalizable", kind=ErrorKind.CONFLICT)
    slots = list(
        session.exec(
            select(CaptureUploadSlot).where(CaptureUploadSlot.inbox_item_id == row.id)
        )
    ...
    if not slots or any(
        ...
        raise OperationError("capture_upload_slots_incomplete", kind=ErrorKind.CONFLICT)
...
def update(
    ...
    if row.state in {
        ...
        raise OperationError("pending_import_not_editable", kind=ErrorKind.CONFLICT)
...
def retry(session: Session, row: InboxItem) -> InboxItem:
    ...
    if (
        ...
        try:
            ...
            raise OperationError("staging_expired", kind=ErrorKind.CONFLICT) from exc
    row.error_code = None
    row.retryable = False
    row.completion = None
    row.updated_at = utcnow()
...
def _dismiss_browser_lease(session: Session, row: InboxItem) -> bool:
    ...
    try:
        ...
        raise OperationError(detail, kind=ErrorKind.CONFLICT) from exc
    except staging_leases.StagingLeaseError as exc:
        detail = (
            "staging_expired" if "expired" in str(exc) else "staging_cleanup_failed"
        )
