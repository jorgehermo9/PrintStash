...
class TestQueryStatus:
    """Every provider's status payload, pinned at the shape callers already read.

    These are characterization tests: their job is not to say the shape is right
    but that it has not *changed*. The frontend, the fleet router and the print
    tracker all read these dictionaries by key, and a provider that renames or
    drops one keeps passing its own tests while breaking all three."""

    @pytest.mark.asyncio
    async def test_moonraker_status_contract_preserves_canonical_wire_shape(
        self,
    ) -> None:
        expected = {
            "result": {
    ...
    @pytest.mark.asyncio
    async def test_prusalink_status_contract_uses_canonical_snapshot_shape(
        self,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_octoprint_status_contract_uses_canonical_snapshot_shape(
        self,
    ) -> None:
        def handler(request: httpx.Request) -> httpx.Response:
    ...
    @pytest.mark.asyncio
    async def test_centauri_status_contract_uses_canonical_snapshot_shape(self) -> None:
...
class TestCaptureArtifact:
    def test_artifact_capture_is_an_optional_bambu_only_extension(self) -> None:
        providers = {
    ...
    def test_bambu_project_request_exposes_current_capture_hint_shape(self) -> None:
        provider = BambuLanProvider("192.0.2.10", "TEST-SERIAL", "test-code")
