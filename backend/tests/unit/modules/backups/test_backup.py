...
import app.db.models as backup_db_models
...
class TestBackupManifestStream:
    def test_nonmanifest_first_member_is_not_discovered(self) -> None:
        ...
        assert backup_archive_format._read_manifest_from_stream(body) is None
    ...
    def test_unreadable_journal_directory_keeps_cache_pinned(
        ...
        assert backup_caches._cache_path_pinned_by_restore_journal("cache") is True
    ...
    def test_source_ref_pins_complete_object_identity(self) -> None:
        first = backup_targets.source_reference(
            location="s3", provider_ref="a" * 64, namespace="bucket/prefix", path="k"
        )
        assert first != backup_targets.source_reference(
...
class TestRestoreJournalStateMachine:
    def test_new_journal_binds_current_provider_to_started_lifecycle_binding(
        ...
        monkeypatch.setattr(backup_restore_journal, "get_backend", lambda: backend)
        path = tmp_path / ".restore-provider-bound.journal"

        state = backup_restore_journal._prepare_restore_journal(
            path,
        ...
        assert backup_restore_journal._journal_binding(state.started) == {
            "backup_id": "provider-bound",
            "operation_nonce": "b" * 64,
            "archive_sha256": "a" * 64,
    ...
    def test_prepare_rejects_a_resumed_journal_bound_to_another_provider(
        ...
        with pytest.raises(
            backup_maintenance.RestoreConflictError, match="restore_journal_mismatch"
        ):
            backup_restore_journal._prepare_restore_journal(
                path,
                backup_id="provider-switch",
                archive_sha256="a" * 64,
                blobs=[],
    ...
    def test_restore_rejects_a_provider_switch_before_discovery_or_mutation(
        ...
        monkeypatch.setattr(backup_restore_journal, "get_backend", lambda: new_backend)
        monkeypatch.setattr(
            backup_snapshot, "_require_database_backup_support", lambda **_: None
        )
        discovered = False
        ...
        monkeypatch.setattr(backup_catalogue, "get_backup", fail_discovery)
    ...
    def test_rejects_complete_before_database_active(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(
            ...
            backup_restore_journal._load_restore_journal(path)
...
class TestUnsafeMemberName:
    def test_verify_backup_flags_unsafe_member_name(
        ...
        result = backup_verification.verify_backup(_id_from(archive))
...
class TestRestoreJournalV2:
    ...
    def test_returns_none_when_journal_discovery_fails(
        ...
        assert backup_recovery.unresolved_restore_backup_id() is None
    ...
    def test_routes_a_readable_journal_to_its_filename_identity(
        ...
        assert backup_recovery.unresolved_restore_backup_id() == "routed"
    ...
    def test_upgrades_a_matching_v1_journal_forward_only(self, tmp_path: Path) -> None:
        ...
        state = backup_restore_journal._prepare_restore_journal(  # noqa: SLF001
            path,
            backup_id="abc",
            archive_sha256="a" * 64,
            blobs=[],
    ...
    def test_projects_v1_upgrade_identity_into_marker_proof(
        ...
        state = backup_restore_journal._load_restore_journal(path)
        assert state.started["version"] == 2
    ...
    def test_interrupted_journal_gates_mutations_across_restart(
        ...
        try:
            assert backup_recovery.inspect_restore_recovery() is True
            assert backup_maintenance.restore_in_progress() is True
            assert backup_recovery.unresolved_restore_backup_id() == "resume"
        finally:
            backup_maintenance._restore_gate.clear()
    ...
    def test_no_journal_leaves_restore_maintenance_clear(
        ...
        assert backup_recovery.inspect_restore_recovery() is False
    ...
    def test_unreadable_journal_directory_keeps_pending_state(
        ...
        assert backup_recovery._restore_journal_pending() is True

    def test_rejects_an_unbalanced_mutating_operation(self) -> None:
        backup_maintenance._active_mutations = 0
    ...
    def test_rejects_mutation_while_restore_maintenance_is_active(self) -> None:
        backup_maintenance._restore_gate.set()
        try:
            assert backup_maintenance.begin_mutating_operation() is False
        finally:
            backup_maintenance._restore_gate.clear()
...
class TestBackupStorageHelpers:
    def test_backup_s3_target_repr_does_not_expose_credentials(self) -> None:
        target = backup_targets._BackupS3Target(
            client=object(),
    ...
    def test_s3_archive_download_pins_etag_then_closes_the_body(
        ...
        archive, response = backup_adoption._download_s3_archive(
            target,
    ...
    def test_remote_identity_is_required_before_candidate_validation(self) -> None:
        with pytest.raises(RuntimeError, match="backup_remote_identity_unavailable"):
            backup_targets._require_remote_identity({})
    ...
    def test_unconfigured_remote_provider_has_no_adoption_candidates(
        ...
        assert backup_adoption.discover_unowned_s3_backups() == []
    ...
    def test_missing_local_backup_root_has_no_adoption_candidates(
        ...
        assert backup_adoption.discover_unowned_local_backups() == []
    ...
    def test_rejects_an_unsupported_database_backup(
        ...
        with pytest.raises(
            ...
            backup_snapshot._require_database_backup_support()
    ...
    def test_returns_none_when_cloud_backups_are_not_configured(
        ...
        assert backup_targets._get_backup_s3() is None
    ...
    def test_cache_refreshes_after_backup_bucket_changes(
        ...
        first = backup_targets._get_backup_s3_target()
        monkeypatch.setitem(_overlay, "backup_s3_bucket", "second-bucket")
        second = backup_targets._get_backup_s3_target()
    ...
    def test_cache_refreshes_after_every_target_component_changes(
        ...
        first_target = backup_targets._get_backup_s3_target()
        monkeypatch.setitem(_overlay, field, second)
        second_target = backup_targets._get_backup_s3_target()
    ...
    def test_retries_client_construction_after_transient_failure(
        ...
        assert backup_targets._get_backup_s3_target() is None
        monkeypatch.setitem(_overlay, "backup_s3_bucket", "repaired-bucket")
        repaired = backup_targets._get_backup_s3_target()
    ...
    def test_config_snapshot_is_never_mixed_during_atomic_runtime_updates(
        ...
        thread.start()
        try:
            snapshots.extend(backup_targets._backup_s3_config() for _ in range(2_000))
        finally:
            stop.set()
    ...
    def test_captured_s3_operation_keeps_original_target_after_config_rotation(
        ...
        row = backup_db_models.OwnedStorageObject(
            backend="backup-s3",
            namespace="bucket-a/printstash-backups/",
            key="printstash-backups/archive.tar.gz",
            object_kind="backup",
            state=backup_db_models.StorageObjectState.COMMITTED,
            size_bytes=1,
            etag='"etag"',
        )
        monkeypatch.setitem(_overlay, "backup_s3_bucket", "bucket-b")

        backup_targets._s3_head_owned(target, row)
    ...
    def test_captured_mutable_s3_identity_uses_conditional_etag(
        ...
        assert backup_targets._s3_identity_kwargs(
    ...
    def test_captured_mutable_s3_identity_without_etag_is_rejected(
        ...
        with pytest.raises(
            backup_contracts.BackupOwnershipError, match="remote_identity_unavailable"
        ):
            backup_targets._s3_identity_kwargs(
                bucket="bucket-a",
                key="archive.tar.gz",
    ...
    def test_remote_operation_without_stable_identity_fails_closed(self) -> None:
        ...
        with pytest.raises(
            ...
            backup_targets._s3_object_kwargs(bucket="bucket-a", key=row.key, row=row)
...
class TestListBackupsSafety:
    def test_unknown_hash_collision_is_not_deduplicated(
        ...
        local = backup_contracts.BackupMeta(
            id="same-id",
        ...
        remote = backup_contracts.BackupMeta(
            id="same-id",
        ...
        assert {item.source_ref for item in backup_catalogue.list_backups()} == {
            "local-ref",
            "remote-ref",
        }
    ...
    def test_collision_requires_source_ref_when_archive_hashes_differ(
        ...
        left = backup_contracts.BackupMeta(
            id="same-id",
            created_at="2020-01-01T00:00:00+00:00",
            size_bytes=1,
            storage_backend="local",
        ...
        right = backup_contracts.BackupMeta(
            id="same-id",
            created_at="2020-01-01T00:00:00+00:00",
            size_bytes=2,
            storage_backend="s3",
        ...
        assert {meta.source_ref for meta in backup_catalogue.list_backups()} == {
            "left-ref",
            "right-ref",
        }
    ...
    def test_returns_none_when_cloud_client_initialization_fails(
        ...
        assert backup_targets._get_backup_s3() is None
        assert backup_targets._backup_s3 is False
    ...
    def test_restores_database_bytes_through_a_temporary_snapshot(
        ...
        monkeypatch.setattr(backup_snapshot, "_db_path", lambda: target)
    ...
    def test_uses_engine_fallback_when_factory_has_no_dispose(
        ...
        engine = _Engine()
        monkeypatch.setattr(backup_snapshot, "get_session_factory", lambda: object())
        monkeypatch.setattr(backup_snapshot, "get_engine", lambda: engine)

        backup_snapshot._dispose_session_engine()
    ...
    def test_returns_unknown_when_active_marker_read_fails(
        ...
        monkeypatch.setattr(backup_restore_journal, "get_session_factory", fail_factory)
    ...
    def test_removes_a_completed_journal_file(self, tmp_path: Path) -> None:
        ...
        backup_restore_journal._remove_restore_journal(path)
    ...
    def test_rejects_a_journal_event_without_a_generation(self) -> None:
        with pytest.raises(
            ...
            backup_restore_journal._journal_generation({})

    def test_skips_a_backup_with_an_invalid_creation_timestamp(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        candidate = backup_contracts.BackupMeta(
            id="invalid-date",
        ...
        monkeypatch.setattr(backup_catalogue, "list_backups", lambda: [candidate])

        assert backup_deletion.purge_old_backups(retain_days=1) == 0
