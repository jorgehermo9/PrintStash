...
conversion and is served straight from the Artifact.
...
While the conversion is pending the endpoint answers `202` with the derivative's
state, and the viewer waits for it. Oversized, timed-out or invalid files show an
error without displaying a partial conversion. Temporary copies are removed after
