...
def _cursor_page_ids(
    ...
    *,
    ...
    total = 0
    while True:
        page = models_pagination.page_items(
            db_session,
            superuser,
...
class TestListItems:
    def test_pagination_visits_every_model_once_when_timestamps_tie(
        ...
        seen = _paginate_ids(
            lambda limit, offset: models_listing.list_items(
    ...
    def test_list_items_excludes_external_sentinel(
        ...
        items = models_listing.list_items(db_session, superuser, limit=100)
    ...
    def test_list_items_includes_daily_workflow_print_outcomes(
        ...
        item = next(
            row
            for row in models_listing.list_items(db_session, superuser, limit=100)
            if row.id == model.id
        )
    ...
    def test_cursor_page_rejects_sort_mismatch(
        ...
        first = models_pagination.page_items(
            db_session,
            superuser,
            filters=ModelFilters(),
...
class TestListTrashed:
    def test_trashed_pagination_visits_every_model_exactly_once(
        ...
        seen = _paginate_ids(
            lambda limit, offset: models_trash.list_trashed(
