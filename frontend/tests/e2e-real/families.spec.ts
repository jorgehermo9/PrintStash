...
test.describe("Manual Model Families", () => {
  test("preserves independent Benchy variants in a manual Family", async ({ page }, testInfo) => {
    ...
    try {
      ...
      for (const scale of [1, 0.5]) {
        ...
        const mesh = readFileSync(
          new URL("../../../testdata/benchy/3dbenchy.stl", import.meta.url),
        );
        ...
        const gcode = Buffer.concat([
          readFileSync(
            new URL(
              "../../../backend/tests/fixtures/real_orca_ender3_benchy.gcode",
              import.meta.url,
            ),
          ),
          Buffer.from(`\n; Family fixture: ${name}\n`),
      ...
      await expect(
        create.getByRole("button", { name: "Create Family", exact: true }),
      ).toBeDisabled();
      ...
      await page.getByRole("button", { name: "Compare two Models (2/2)", exact: true }).click();
      await expect(
        comparison.getByText("Failed to load 3D preview", { exact: true }),
      ).toBeVisible();
  ...
  test("builds Multipart Choices from explicitly regrouped Models", async ({ page }) => {
    ...
    try {
      ...
      const seeded = await page.request.put(
        `${API}/api/v1/multipart-models/${multipart.id}/parts`,
        {
          data: {
      ...
      await choices
        .getByRole("checkbox", { name: `Select ${models[2].name}`, exact: true })
        .check();
      ...
      for (const id of families) {
        ...
        await page.request.delete(
          `${API}/api/v1/families/${id}/purge?version=${family.version + 1}`,
        );
      }
      for (const model of models) await page.request.delete(`${API}/api/v1/models/${model.id}`);
    }
