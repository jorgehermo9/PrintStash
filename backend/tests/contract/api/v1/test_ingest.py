...
class TestImportFromUrl:
    ...
    def test_collection_auto_import_creates_models_per_member(
        ...
        members = [
            import_resolvers.CollectionMember(
                page_url="https://www.printables.com/model/1-a",
                title="A",
                source_id="1",
            ),
            import_resolvers.CollectionMember(
                page_url="https://www.printables.com/model/2-b",
                title="B",
                source_id="2",
            ),
        ]
    ...
    def test_collection_review_then_select_imports_chosen_member(
        ...
        models = db_session.exec(
            select(Model).where(Model.source_url.is_not(None))
        ).all()
        assert any(
            m.source_url == "https://www.printables.com/model/1-a" for m in models
        )
        ...
        with (
            ...
            manifest = _job(
                ...
                client.post(
                    ...
                    headers=auth_headers,
                    json={
                        "url": "https://www.printables.com/model/1660232-springy-cat"
                    },
                ),
                auth_headers,
            )
