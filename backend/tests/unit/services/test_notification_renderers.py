...
class TestRenderWebhook:
    def test_webhook_wraps_full_context(self):
        import json
    ...
    def test_webhook_hmac_signature_when_secret_set(self):
        import hashlib
        import hmac
...
class TestRenderTelegram:
    ...
    def test_telegram_preserves_app_api_base_override(self, monkeypatch):
    ...
    def test_telegram_escapes_html_special_chars_in_names(self):
    ...
    def test_telegram_omits_link_when_no_model_url(self):
        req = r.render_telegram(
            _ctx(model_url=None), {"bot_token": "t", "chat_id": "c"}
        )
        assert "View model" not in req.json["text"]

    def test_telegram_rejects_non_http_model_url(self):
...
class TestRenderNtfy:
    ...
    def test_ntfy_title_with_unicode_name_is_header_safe(self):
    ...
    def test_ntfy_omits_click_when_no_model_url(self):
...
class TestSummaryLines:
    ...
    def test_summary_skips_absent_optional_fields(self):
...
class TestRegistry:
    ...
    def test_registry_covers_every_target(self, target):
        assert target in r.RENDERERS

    def test_facade_keeps_orm_enums_separate_from_core_contracts(self):
