...
from tests.e2e._jobs import completed_job, create_backup, settle
...
async def ingest(api, headers, name: str, payload: bytes):
    ...
    await completed_job(api, response, headers)
...
class TestVaultMigrationWorkflow:
    ...
    async def test_failed_candidate_never_replaces_source_downloads(
        self, api, e2e_db, superuser_headers, tmp_path, migration_runtime
    ):
    ...
    async def test_verified_activation_downloads_every_online_delta_artifact(
        ...
        for row in e2e_db.exec(select(File)).all():
            ...
            response = await api.get(
                f"/api/v1/files/{row.id}/download", headers=headers
            )
            assert response.status_code == 200, response.text
        ...
        fresh = await create_backup(api, headers)
        cleanup = await api.post(
            ...
            json={
                "confirmation": plan["id"],
                "source": True,
                "backup_id": fresh["backup_id"],
                "backup_source_ref": fresh["source_ref"],
            },
        )
