...
def test_provenance_detail_returns_safe_effective_field_and_capture_summaries(
    ...
    assert {key: read[key] for key in ("id", "provider", "source_item_id", "canonical_url", "source_revision")} == {
        "id": source.id,
        "provider": "printables",
    ...
    assert {key: read["captures"][0][key] for key in ("adapter_version", "source_revision", "snapshot_sha256")} == {
        "adapter_version": "printables-v1",
        "source_revision": None,
        "snapshot_sha256": "c" * 64,
    }
...
def test_provenance_override_and_clear_are_edit_scoped_and_leave_legacy_source_url_alone(
    ...
    title = next(item for item in updated.json()["sources"] if item["id"] == source.id)["fields"][0]
    ...
    assert client.get(f"/api/v1/models/{model.id}", headers=auth_headers).json()["source_url"] == (
    ...
    title = next(item for item in cleared.json()["sources"] if item["id"] == source.id)["fields"][0]
