...
class TestSanitizeCanonicalUrl:
    def test_keeps_an_ordinary_https_url(self) -> None:
        assert (
            sanitize_canonical_url("https://example.test/model/1")
            == "https://example.test/model/1"
        )

    def test_discards_the_query_string(self) -> None:
        # A query can carry a signed credential or tracking; neither belongs in
        # a row that lives forever.
        assert (
            sanitize_canonical_url("https://example.test/a?token=secret")
            == "https://example.test/a"
        )

    def test_discards_the_fragment(self) -> None:
        assert (
            sanitize_canonical_url("https://example.test/a#files")
            == "https://example.test/a"
        )
    ...
    def test_encodes_an_internationalized_hostname(self) -> None:
        # Stored in one form only, so a lookup by host cannot miss.
        assert sanitize_canonical_url("https://bücher.test/a") == (
            "https://xn--bcher-kva.test/a"
        )

    def test_keeps_a_port_for_a_provider_it_does_not_know(self) -> None:
        # Portable captures from a self-hosted source may legitimately use one.
        assert (
    ...
    def test_brackets_an_ipv6_host(self) -> None:
        assert sanitize_canonical_url("https://[::1]/a") == "https://[::1]/a"
    ...
    def test_refuses_anything_that_is_not_an_absolute_http_url(self, url: str) -> None:
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url(url)
    ...
    def test_refuses_credentials_embedded_in_the_url(self, url: str) -> None:
        # Userinfo is a credential, and it also changes which host a browser
        # actually contacts in some readings.
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url(url)

    def test_refuses_a_malformed_authority(self) -> None:
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url("https://example.test:notaport/a")

    def test_refuses_a_hostname_that_cannot_be_encoded(self) -> None:
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url("https://" + "ü" * 300 + ".test/a")

    def test_refuses_a_url_over_the_length_cap(self) -> None:
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url("https://example.test/" + "a" * 3000)

    def test_refuses_a_value_that_is_not_a_string(self) -> None:
        with pytest.raises(CaptureContractError):
            sanitize_canonical_url(None)


class TestCanonicalizeProviderUrl:
    def test_returns_a_sanitized_url_for_an_unknown_provider(self) -> None:
        # PrintStash accepts portable captures from providers it has no adapter
        # for; it just cannot bind their routes.
        assert (
            canonicalize_provider_url("selfhosted", "https://example.test/x?a=1")
    ...
    @pytest.mark.parametrize(
        ("provider", "url"),
        [
            ("printables", "https://www.printables.com/model/123-example"),
            ("printables", "https://printables.com/model/123-example/files"),
            ("makerworld", "https://makerworld.com/models/123-example"),
            ("makerworld", "https://makerworld.com/en/models/123-example"),
            ("makerworld", "https://assets.makerworld.com/en/models/123"),
            ("thingiverse", "https://www.thingiverse.com/thing:123"),
            ("thingiverse", "https://www.thingiverse.com/thing:123/files"),
            ("thingiverse", "https://www.thingiverse.com/things/123"),
            ("thingiverse", "https://www.thingiverse.com/things/123/files"),
            ("cults", "https://cults3d.com/3d-model/gadget/thing"),
            ("cults", "https://cults3d.com/en/3d-model/gadget/thing"),
            ("myminifactory", "https://www.myminifactory.com/object/123-example"),
        ],
    )
    def test_accepts_every_real_page_route_for_a_known_provider(
        self, provider: str, url: str
    ) -> None:
        assert canonicalize_provider_url(provider, url)
    ...
    def test_refuses_a_path_a_proxy_might_read_differently(self, path: str) -> None:
        # If the decoded path differs from the validated one, the durable
    ...
    @pytest.mark.parametrize(
        "url",
        [
            "https://printables.com/collections/123",
            "https://printables.com/model/123/comments",
            "https://printables.com/@user",
            "https://printables.com/model/123/files/extra",
        ],
    )
    def test_refuses_a_printables_url_that_is_not_a_model_page(self, url: str) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url("printables", url)

    def test_normalizes_the_printables_route_token_case(self) -> None:
        # Route tokens are case-insensitive to the provider; the slug is not.
        assert (
            canonicalize_provider_url(
                "printables", "https://printables.com/MODEL/123-Example/FILES"
            )
            == "https://printables.com/model/123-Example/files"
        )

    def test_drops_a_trailing_slash(self) -> None:
        assert (
            canonicalize_provider_url("printables", "https://printables.com/model/123/")
            == "https://printables.com/model/123"
        )

    def test_lower_cases_a_makerworld_locale(self) -> None:
        assert (
            canonicalize_provider_url(
    ...
    def test_refuses_a_makerworld_locale_that_is_not_one(self) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url(
                "makerworld", "https://makerworld.com/notalocale/models/1"
            )

    def test_refuses_a_cults_locale_that_is_not_one(self) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url("cults", "https://cults3d.com/eng/3d-model/a/b")

    def test_refuses_a_cults_path_missing_its_slug(self) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url("cults", "https://cults3d.com/3d-model/a")

    def test_refuses_a_myminifactory_route_that_is_not_an_object(self) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url(
                "myminifactory", "https://myminifactory.com/users/1"
            )

    def test_normalizes_the_thingiverse_colon_form(self) -> None:
        assert (
            canonicalize_provider_url(
                "thingiverse", "https://thingiverse.com/THING:123/FILES"
            )
            == "https://thingiverse.com/thing:123/files"
        )

    def test_refuses_a_thingiverse_id_that_is_not_numeric(self) -> None:
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url(
                "thingiverse", "https://thingiverse.com/things/abc"
            )
    ...
    def test_refuses_a_url_naming_a_different_model_than_the_capture(self) -> None:
        # A mismatch means the metadata and the page have been mixed up, which
        # would attribute one model's licence to another.
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url(
                "printables", "https://printables.com/model/999-other", "123"
            )

    def test_refuses_an_id_that_is_only_a_numeric_prefix_of_the_slug(self) -> None:
        # `12` must not match `123-example`, or captures would collide.
        with pytest.raises(CaptureContractError):
            canonicalize_provider_url(
                "printables", "https://printables.com/model/123-example", "12"
            )

    def test_is_the_same_function_under_the_sanitize_name(self) -> None:
        # Two vocabularies, one implementation — so the rules cannot diverge.
