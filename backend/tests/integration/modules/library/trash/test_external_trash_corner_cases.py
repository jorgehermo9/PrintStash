...
``integration/modules/sources/external_library/`` by pinning down the *mixed model* and
...
class TestTrashExpiresAt:
    def test_trash_expires_at_adds_retention_window(self) -> None:
        deleted = datetime(2026, 6, 1, 12, 0, tzinfo=timezone.utc)
        assert trash.trash_expires_at(deleted, 30) == deleted + timedelta(days=30)

    def test_trash_expires_at_zero_retention_is_immediate(self) -> None:
        deleted = datetime(2026, 6, 1, 12, 0, tzinfo=timezone.utc)
        assert trash.trash_expires_at(deleted, 0) == deleted

    def test_trash_expires_at_disabled_or_live_returns_none(self) -> None:
        deleted = datetime(2026, 6, 1, 12, 0, tzinfo=timezone.utc)
