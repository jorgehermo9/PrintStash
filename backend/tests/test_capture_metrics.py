...
def test_capture_metrics_allowlist_malicious_labels() -> None:
    ...
    record_capture_operation(hostile, hostile, hostile, 0.1, uploaded_bytes=7, error_category=hostile)
...
def test_provider_resolver_emits_one_bounded_success_observation(monkeypatch) -> None:
    ...
    async def metadata(*_args):
        return ProviderModelMetadata("1", "Widget", None, None, None, ())
    monkeypatch.setattr(import_resolvers.provider_connections, "fetch_mmf_model_metadata", metadata)
    asyncio.run(import_resolvers.resolve_connected_provider_capture("https://www.myminifactory.com/object/1", import_resolvers.ProviderResolutionContext(1, cast(SessionFactory, Factory()))))
    text = generate_latest(registry).decode()
...
def test_upload_transports_and_manual_file_error_are_bounded() -> None:
    record_capture_operation("makerworld", "browser_upload", "success", 0, uploaded_bytes=3)
    record_capture_operation("makerworld", "upload_slots", "success", 0, uploaded_bytes=4)
    record_capture_operation("cults", "provider_api", "required", 0, error_category="user_file_required")
    text = generate_latest(registry).decode()
