...
import app.modules.backups.backup.archive_format as backup_archive_format
...
import app.modules.backups.backup.downloads as backup_downloads
import app.modules.backups.backup.publications as backup_publications
import app.modules.backups.backup.recovery as backup_recovery
import app.modules.backups.backup.restore as backup_restore
import app.modules.backups.backup.restore_blobs as backup_restore_blobs
import app.modules.backups.backup.restore_journal as backup_restore_journal
...
import app.modules.storage.storage_backend.contracts as storage_contracts
import app.modules.storage.storage_backend.local as storage_local
import app.modules.storage.storage_ownership as backup_ownership
...
def _rewrite_backup_archive(
    ...
    *,
    ...
    from app.modules.storage.storage_ownership import replace_owned_bytes
...
def _return_invalid_size_receipt(
    backend: storage_contracts.StorageBackend,
    _real_create: Callable,
    content: bytes,
    _source,
    destination: str,
):
    return storage_contracts.CreationReceipt(
        key=destination,
        size=len(content) + 1,
        token="invalid-size",
...
def _publish_wrong_hash(
    _backend: storage_contracts.StorageBackend,
...
class TestBackupSqliteCopy:
    def test_backup_sqlite_copy_contains_committed_model(
        ...
        snapshot = backup_snapshot._backup_sqlite_copy()
    ...
    def test_backup_sqlite_copy_raises_for_non_file_db(
        ...
        monkeypatch.setattr(backup_snapshot, "_db_path", lambda: None)
        with pytest.raises(RuntimeError, match="not a file-based SQLite database"):
            backup_snapshot._backup_sqlite_copy()
...
class TestCreateBackup:
    ...
    def test_create_backup_fails_if_blob_vanishes_mid_write(
        ...
        monkeypatch.setattr(backup_snapshot, "_add_file_to_tar", _boom)
    ...
    def test_create_backup_raises_when_blob_size_changes_mid_archive(
        ...
        real_find_blobs = backup_snapshot._find_blobs

        def _wrong_size(session: Session | None = None):
            return [(k, size + 999) for k, size in real_find_blobs(session)]

        monkeypatch.setattr(backup_snapshot, "_find_blobs", _wrong_size)
    ...
    def test_create_backup_rejects_same_size_content_change_during_streaming(
        ...
        real_add = backup_snapshot._add_file_to_tar
        ...
        monkeypatch.setattr(backup_snapshot, "_add_file_to_tar", replace_before_stream)
    ...
    def test_backup_id_round_trips_despite_timestamped_name(
        ...
        fetched = backup_catalogue.get_backup(meta.id)
    ...
    def test_creates_a_local_archive_before_uploading_its_cloud_copy(
        ...
        assert store.upload_observations == [
            (f"{backup_contracts._BACKUP_S3_PREFIX}{Path(meta.path).name}", True)
        ]
        with backup_env.new_session() as session:
            rows = session.exec(
                select(OwnedStorageObject).where(
...
class TestValidateCreatedArchivePayload:
    ...
    def test_completed_archive_rejects_invalid_payload(
        ...
        with pytest.raises(error_type, match=error):
            backup_snapshot._validate_created_archive_payload(archive)
...
class TestListLocalBackups:
    def test_list_local_backups_empty_when_dir_missing(self, backup_env: BackupEnv):
        ...
        assert backup_catalogue._list_local_backups() == []
        assert backup_catalogue.list_backups() == []
    ...
    def test_legacy_archive_requires_explicit_adoption(self, backup_env: BackupEnv):
        ...
        assert backup_catalogue.list_backups() == []
        adopted = backup_adoption.adopt_local_backup(archive.name)
        assert adopted.id == meta.id
        assert {item.id for item in backup_catalogue.list_backups()} == {meta.id}
    ...
    def test_legacy_null_receipt_is_unusable_until_full_adoption(
        ...
        assert backup_catalogue.list_backups() == []
        assert backup_deletion.delete_backup(meta.id) is False
        with pytest.raises(FileNotFoundError):
        ...
        adopted = backup_adoption.adopt_local_backup(archive.name)
        assert adopted.id == meta.id
        assert backup_catalogue.get_backup(meta.id) is not None
        assert backup_deletion.delete_backup(meta.id) is True
    ...
    def test_cloud_cache_is_not_a_listable_backup_source(
        ...
        assert backup_catalogue.list_backups() == []
        assert backup_catalogue.list_backup_sources() == []
        assert cache.exists()
    ...
    def test_unowned_discovery_returns_only_valid_adoption_candidates(
        ...
        candidates = backup_adoption.discover_unowned_local_backups()
...
class TestUploadBackupArchive:
    ...
    def test_rejects_an_unsafe_or_unrecognized_filename(
        ...
        with pytest.raises(ValueError, match="backup_filename_invalid"):
            backup_adoption.upload_backup_archive(filename, io.BytesIO(b"unused"))
...
class TestListBackups:
    def test_list_backups_skips_archive_with_unreadable_manifest(
        ...
        seed_model_with_blob(backup_env, name="Widget", content=b"x")
        good = backup_creation.create_backup()
        ...
        listed = {m.id for m in backup_catalogue.list_backups()}
    ...
    def test_list_backups_prefers_the_local_row_on_an_id_collision(
        ...
        seed_model_with_blob(backup_env, name="Widget", content=b"x")
        local_meta = backup_creation.create_backup()

        s3_only = backup_contracts.BackupMeta(
            id="s3-only-id",
        ...
        s3_dupe = backup_contracts.BackupMeta(
            id=local_meta.id,
    ...
    def test_list_backups_includes_a_backup_that_exists_only_in_the_cloud(
        ...
        cloud_only = backup_contracts.BackupMeta(
            id="cloud-only-id",
            created_at="2020-01-01T00:00:00+00:00",
            size_bytes=123,
        ...
        duplicate_of_local = backup_contracts.BackupMeta(
            id=local_meta.id,
    ...
    def test_a_new_backup_appears_in_the_listing(self, backup_env: BackupEnv):
        ...
        assert any(m.id == meta.id for m in backup_catalogue.list_backups())
...
class TestReadManifest:
    def test_read_manifest_returns_none_when_manifest_entry_is_a_directory(
        ...
        assert backup_archive_format._read_manifest(gz_archive) is None
    ...
    def test_read_manifest_returns_none_when_manifest_member_unreadable(
        ...
        assert backup_archive_format._read_manifest(archive) is None
...
class TestGetBackupArchivePath:
    def test_get_backup_archive_path_raises_for_unknown_id(self, backup_env: BackupEnv):
        with pytest.raises(FileNotFoundError):
            backup_catalogue.get_backup_archive_path("does-not-exist")
...
class TestVerifyBackup:
    def test_verify_backup_accepts_an_intact_archive(self, backup_env: BackupEnv):
        ...
        result = backup_verification.verify_backup(meta.id)
...
class TestVerifyBackupOwnership:
    @staticmethod
    def _verification(*, valid: bool) -> backup_contracts.BackupVerification:
    ...
    def test_missing_ledger_row_is_reported_without_discovery(self) -> None:
        result = backup_verification.verify_backup_ownership(987654)
    ...
    def test_available_opendal_destination_verifies_its_owned_archive(
        ...
        monkeypatch.setattr(backup_caches, "cleanup_backup_cache", cleaned.append)
    ...
    def test_unrecognized_archive_name_uses_the_ledger_identity(
        ...
        ) -> backup_contracts.BackupVerification:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                backup_contracts.BackupOwnershipError("backup_archive_digest_mismatch"),
                "digest",
    ...
    def test_remote_receipt_preserves_failure_classification(
        ...
        def fail(_meta: backup_contracts.BackupMeta) -> Path:
            raise failure

        monkeypatch.setattr(backup_downloads, "_download_backup_to_local", fail)
...
class TestDeleteBackup:
    def test_delete_backup_removes_archive(self, backup_env: BackupEnv):
        ...
        assert backup_catalogue.get_backup(meta.id) is None
    ...
    def test_cleanup_keeps_a_cache_pinned_by_restore_journal(
        ...
        provider_ref = backup_restore_journal._restore_provider_ref()
        journal.write_text(
            "\n".join(
    ...
    def test_delete_backup_removes_s3_copy(self, backup_s3_env: BackupEnv):
        ...
        cloud = next(
            source
            for source in backup_catalogue.list_backup_sources()
...
class TestBackupCacheRecovery:
    ...
    def test_startup_cache_cleanup_retires_only_exact_stale_owned_cache(
        ...
        assert backup_caches.reconcile_backup_caches() == 2
    ...
    def test_unprovable_cache_is_blocked_without_deleting_bytes(
        ...
        assert backup_caches.reconcile_backup_caches() == 0
    ...
    def test_stale_cache_delete_failure_preserves_bytes_for_recovery(
        ...
        monkeypatch.setattr(backup_caches, "delete_owned_key", fail_delete)
    ...
    def test_malformed_restore_journal_never_pins_an_unrelated_cache(
        ...
        assert backup_caches._cache_path_pinned_by_restore_journal("unrelated") is False
    ...
    def test_ownership_queries_keep_provider_namespaces_separate(
        ...
        assert backup_catalogue._committed_backup_keys(
            "backup-s3",
            namespace="first/printstash-backups/",
            provider_ref="first-provider",
        ) == {first_key}
        rows = backup_catalogue._backup_ownership_rows(
    ...
    def test_cache_path_encodes_complete_safe_source_identity(
        ...
        for source_ref, version_id in (
            ...
            monkeypatch.setattr(backup_targets, "_get_backup_s3", lambda s=store: s)
            remote = OwnedStorageObject(
                backend="backup-s3",
                namespace="archive-bucket/printstash-backups/",
            ...
            paths.append(backup_downloads._download_backup_to_local(meta))
...
class TestHasMember:
    def test_has_member_false_for_missing_entry(self, backup_env: BackupEnv):
        ...
        with tarfile.open(Path(meta.path), mode="r:gz") as tar:
            assert backup_archive_format._has_member(tar, "manifest.json") is True
            assert backup_archive_format._has_member(tar, "nonexistent.entry") is False
...
class TestRestoreKeyMap:
    def test_restore_key_map_empty_for_legacy_archive_without_manifest(
        ...
        with tarfile.open(archive, mode="r") as tar:
            assert backup_archive_format._restore_key_map(tar) == {}
...
class TestStageRestoreArchive:
    ...
    def test_restore_accepts_a_v1_archive(
        ...
        with backup_env.new_session() as session:
            ...
            row.sha256 = backup_archive_format._sha256_path(archive)
        ...
        monkeypatch.setattr(backup_restore, "run_migrations", lambda _url: None)
...
class TestRestoreDatabase:
    ...
    def test_start_only_journal_does_not_promote_a_matching_stale_marker(
        ...
        state = backup_restore_journal._load_restore_journal(journal)
    ...
    def test_terminal_journal_requires_matching_active_marker(
        ...
        meta = backup_creation.create_backup()
        Path(key).unlink()
        real_remove = backup_restore_journal._remove_restore_journal
    ...
    def test_restore_database_raises_for_non_file_db(
        ...
        with pytest.raises(RuntimeError, match="cannot restore to non-file database"):
            backup_snapshot._restore_database(b"irrelevant")
    ...
    def test_failed_restore_writes_failed_row(
        ...
        monkeypatch.setattr(backup_downloads, "_download_backup_to_local", _boom)
    ...
    def test_restore_rejects_a_duplicate_destination_before_publish(
        ...
        blob = backup_contracts._StagedBlob(
            key=key,
            path=staged,
            size=len(content),
            sha256="0" * 64,
    ...
    def test_restore_finishes_forward_when_database_swap_reports_after_commit(
        ...
        meta = backup_creation.create_backup()
        Path(key).unlink()
        real_restore = backup_snapshot._restore_database_from_path
    ...
    def test_restore_preserves_final_destination_mutation(
        ...
        meta = backup_creation.create_backup()
        Path(key).unlink()
        real_apply = backup_restore_blobs._apply_staged_blobs
    ...
    def test_restore_reloads_the_latest_valid_publication_generation(
        ...
        backend = get_backend()
        real_create = backend.create_stream
        first_receipts: list[storage_contracts.CreationReceipt] = []
    ...
    def test_restore_rejects_a_corrupt_journal_sequence(
        ...
        with pytest.raises(
            backup_maintenance.RestoreConflictError, match="restore_journal_invalid"
        ):
            backup_restore.restore_backup(meta.id)
    ...
    def test_restore_rejects_a_mismatched_journal(
        ...
        with pytest.raises(backup_maintenance.RestoreConflictError, match=error):
            backup_restore.restore_backup(meta.id)
    ...
    def test_restore_maintenance_waits_for_admitted_mutation(
        ...
        try:
            for _ in range(100):
                if backup_maintenance.restore_in_progress():
                    break
                time.sleep(0.01)
            assert backup_maintenance.restore_in_progress() is True
        ...
        assert backup_maintenance._active_mutations == 0
        assert backup_maintenance.restore_in_progress() is False
...
class TestPurgeOldBackups:
    def test_retention_treats_unsupported_deletion_as_steady_state(
        ...
        def delete(identity, *, source_ref):
            ...
            if identity == "unsupported":
                raise backup_contracts.BackupDeleteUnsupportedError()
            return True
        ...
        assert backup_deletion.purge_old_backups(30) == 1
        assert backup_deletion.purge_old_backups(30) == 1
    ...
    def test_purge_old_backups_noop_when_retention_non_positive(
        ...
        seed_model_with_blob(backup_env, name="Widget", content=b"x")
        backup_creation.create_backup()
        assert backup_deletion.purge_old_backups(retain_days=0) == 0
        assert backup_deletion.purge_old_backups(retain_days=-5) == 0
    ...
    def test_purge_old_backups_skips_entry_with_invalid_created_at(
        ...
        manifest = {
            "version": backup_contracts.MANIFEST_VERSION,
        ...
        removed = backup_deletion.purge_old_backups(retain_days=30)

        assert removed == 0
        assert "badc0ffeeb00" not in {m.id for m in backup_catalogue.list_backups()}
    ...
    def test_purge_keeps_fresh_removes_old(
        ...
        monkeypatch.setattr(backup_creation, "utcnow", utcnow)
        fresh = backup_creation.create_backup()

        removed = backup_deletion.purge_old_backups(retain_days=30)

        assert removed == 1
        remaining = {m.id for m in backup_catalogue.list_backups()}
...
class TestDocument:
    ...
    def test_backup_includes_embedded_document_images(self, backup_env: BackupEnv):
        from app.modules.storage.storage_ownership import record_creation
...
class TestRaises:
    ...
    def test_restore_unknown_backup_raises(self, backup_env: BackupEnv):
        with pytest.raises(FileNotFoundError):
            backup_restore.restore_backup("does-not-exist")
...
class TestDelete:
    def test_delete_unknown_backup_returns_false(self, backup_env: BackupEnv):
        assert backup_deletion.delete_backup("nope") is False
...
class TestDiscoverUnownedS3Backups:
    def test_discovers_an_unowned_s3_archive_with_exact_identity(
        ...
        def pages(**kwargs: object) -> list[dict[str, object]]:
            if kwargs["Prefix"] == backup_contracts._LEGACY_BACKUP_S3_PREFIX:
        ...
        candidates = backup_adoption.discover_unowned_s3_backups()
        ...
        assert candidate["namespace"] == (
            f"archive-bucket/{backup_contracts._LEGACY_BACKUP_S3_PREFIX}"
        )
        assert candidate["archive_sha256"] == hashlib.sha256(payload).hexdigest()
        assert candidate["source_ref"] == backup_targets.source_reference(
            location="s3",
    ...
    def test_current_provider_candidate_is_not_hidden_by_foreign_provider_receipt(
        ...
        meta = backup_creation.create_backup()
        payload = Path(meta.path).read_bytes()
        key = f"{backup_contracts._BACKUP_S3_PREFIX}{Path(meta.path).name}"
        store = _BackupObjectStore(payload, "unused")

        def pages(**kwargs: object) -> list[dict[str, object]]:
            if kwargs["Prefix"] == backup_contracts._BACKUP_S3_PREFIX:
                return [{"Contents": [{"Key": key, "Size": len(payload)}]}]
            return [{"Contents": []}]
        ...
        monkeypatch.setitem(_overlay, "backup_s3_bucket", "archive-bucket")
        monkeypatch.setattr(backup_targets, "_get_backup_s3", lambda: store)
        namespace = f"archive-bucket/{backup_contracts._BACKUP_S3_PREFIX}"
        with backup_env.new_session() as session:
            session.add(
                OwnedStorageObject(
                    backend="backup-s3",
...
class TestAdoptS3Backup:
    ...
    def test_rejects_s3_adoption_when_source_ref_does_not_match_exact_locator(
        ...
        with pytest.raises(ValueError, match="backup_source_ref_mismatch"):
            backup_adoption.adopt_s3_backup(
                key,
    ...
    def test_adopts_an_unowned_s3_archive_with_ledger_identity(
        ...
        adopted = backup_adoption.adopt_s3_backup(
...
class TestReconcileBackupPublications:
    def test_does_not_probe_a_pending_row_in_a_different_configured_bucket(
        ...
        assert backup_publications.reconcile_backup_publications() == 0
        assert calls == []
    ...
    def test_reconciles_a_local_archive_published_before_its_receipt(
        ...
        assert backup_publications.reconcile_backup_publications() == 1
    ...
    def test_reconciles_an_opendal_archive_with_matching_object_proof(
        ...
        reconciled = backup_publications.reconcile_backup_publications()
    ...
    def test_keeps_a_cloud_archive_pending_when_the_provider_is_unavailable(
        ...
        monkeypatch.setattr(backup_targets, "_get_backup_s3", lambda: None)
...
class TestListS3Backups:
    def test_lists_a_committed_cloud_archive_from_both_prefixes(
                ...
                return [
                    {
                        "Contents": [
                            ...
                            {
                                "Key": f"{backup_contracts._BACKUP_S3_PREFIX}unowned.tar.gz",
                                "Size": 1,
                            },
                        ]
                    }
        ...
        listed = backup_catalogue._list_s3_backups()
        ...
        assert set(store.prefixes_seen) == {
            backup_contracts._BACKUP_S3_PREFIX,
            backup_contracts._LEGACY_BACKUP_S3_PREFIX,
        }
