...
### Enum columns: TEXT and a CHECK constraint

Never a database-native enum (`sa.Enum(native_enum=True)`, PostgreSQL `CREATE
TYPE … AS ENUM`): adding a value is `ALTER TYPE … ADD VALUE`, awkward inside a
migration's transaction and impossible to undo, and SQLite has no such type.
Declare an enum column with `app.db.enum_columns`:

```python
from app.db.enum_columns import EnumText, enum_check

class Job(SQLModel, table=True):
    __table_args__ = (enum_check("kind", JobKind),)
    kind: JobKind = Field(sa_column=Column(EnumText(JobKind), nullable=False))
```

`EnumText` is TEXT that binds and loads the enum's members only (an unknown
stored value raises on read). `enum_check` is the named constraint
`ck_<table>_<column>_values`, `CHECK (<column> IN (…))`. Alembic compares
neither enum values nor CHECK constraints by itself; the comparator registered
in `enum_columns` does, so **adding or removing a member is an ordinary
autogenerated migration** (a drop and re-create of the constraint inside
`batch_alter_table`), and `test_models_versus_chain` fails until it exists.
Before removing a member, repair the rows that hold it (see
[Repair before you constrain](#repair-before-you-constrain)).
