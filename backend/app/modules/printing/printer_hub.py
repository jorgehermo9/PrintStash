...
from app.modules.printing.spoolman import SpoolmanClient, SpoolmanError
...
from app.runtime.realtime import InProcessBus, MessageSink, RealtimeBus

from .unattributed import ensure_unattributed_artifact
...
def _bambu_project_task_transition(
    ...
    return candidate_identity["external_project_id"] != incoming["external_task_id"]
...
class PrinterHub:
    ...
    async def attach(self, printer_id: int, ws: MessageSink) -> None:
        await self.bus.subscribe(self._channel(printer_id), ws)
        snap = self.snapshots.get(printer_id)
        if snap is not None:
            try:
                await ws({"type": "snapshot", "printer_id": printer_id, "data": snap})
            except Exception:  # noqa: BLE001 — best-effort initial send; drop on failure
                pass

    async def detach(self, printer_id: int, ws: MessageSink) -> None:
        ...
        with self._session_factory.session() as session:
            ...
            if job is None:
                ...
                if ms_state in (
                    ...
                    sentinel_file_id, sentinel_model_id = ensure_unattributed_artifact(
                        session
                    )
                    job = PrintJob(
                        printer_id=printer_id,
