...
class TestEvent:
    @pytest.mark.asyncio
    async def test_subscription_normalizes_status_and_honors_stop_event(self) -> None:
...
class TestUpload:
    @pytest.mark.asyncio
    async def test_upload_uses_control_enabled_connection_and_returns_remote_name(
        self,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_upload_without_control_enabled_is_rejected(self) -> None:
...
class TestRaises:
    @pytest.mark.asyncio
    async def test_cc2_without_access_code_raises_credentials_missing(self) -> None:
...
class TestSubscribeStatus:
    @pytest.mark.asyncio
    async def test_subscribe_status_wraps_printer_error_from_watch(self) -> None:
        connection = FakeConnection()
    ...
    @pytest.mark.asyncio
    async def test_subscribe_status_wraps_oserror_from_watch(self) -> None:
        connection = FakeConnection()
    ...
    @pytest.mark.asyncio
    async def test_subscribe_status_close_failure_is_swallowed(self) -> None:
        connection = FakeConnection()
...
class TestInfo:
    @pytest.mark.asyncio
    async def test_info_returns_provider_details(self) -> None:
        client = ElegooCentauriClient(
