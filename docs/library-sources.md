...
Reads compare every available size, modification time, ETag and version before
and after downloading. A supplied version or conditional read is used when the
transport supports it. Changed or incomplete reads are discarded before any
Model, Artifact or thumbnail is indexed. Unknown modification times stay unknown.

These checks are snapshots, not a remote filesystem transaction. A service with
no immutable version or conditional read can change and change back between
observations. An SFTP server may expose only second-resolution modification
times. Such changes can escape immediate metadata detection; the hash policy
below provides the eventual check. Back up externally owned originals separately.
...
- a changed size, modification time, ETag or version triggers revalidation on
  the next scan, including equal-size replacements
- sources without a usable modification time, ETag or non-null version are
  downloaded and hashed on every scan
- sources whose supplied markers remain unchanged are still downloaded and
  hashed at least once every seven days
